...
(defn babashka-runtime-layers
  []
  (when-let [layers (seq (filterv (fn [stack] (s/includes? (:StackName stack) BABASHKA_LAYER_INSTANCE))
                                  (:Stacks (cloudformation-description))))]
    (println layers)
    ;; (let [ARN (some-> mstacks
    ;;                   first
...
(println (babashka-runtime-layers))
